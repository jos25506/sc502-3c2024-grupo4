...
<section class="container py-5">
    ...
    <div class="row g-4">
        <?php while ($row = $ofertas->fetch_assoc()): ?>
            ...
            <div class="col-md-4">
                <div class="card oferta-card position-relative shadow">
                    <span class="oferta-badge">-25% OFF</span>
                    <img src="<?= $row['imagen'] ?>" class="card-img-top" alt="Producto">
                    <div class="card-body text-center">
                        <h5 class="fw-bold"><?= $row['nombre_producto'] ?></h5>
                        <p class="mb-1 text-muted text-decoration-line-through">₡<?= number_format($row['precio']) ?></p>
                        <p class="text-success fs-4 fw-bold">₡<?= number_format($precioOferta) ?></p>

                        <?php if (isset($_SESSION['rol']) && $_SESSION['rol'] === 'cliente'): ?>
                            <a href="carrito.php?id=<?= $row['id_producto'] ?>" class="btn btn-primary">🛒 Agregar al carrito</a>
                        <?php endif; ?>

                        <?php if (isset($_SESSION['rol']) && $_SESSION['rol'] === 'admin'): ?>
                            <div class="d-flex justify-content-center gap-2 mt-2">
                                <a href="php/botones/editar_producto.php?id=<?= $row['id_producto'] ?>" class="btn btn-warning btn-sm">Editar</a>
                                <a href="php/botones/eliminar_producto.php?id=<?= $row['id_producto'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('¿Seguro que deseas eliminar este producto?');">Eliminar</a>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <div class="row g-4">
        <?php while ($p = $recomendados->fetch_assoc()): ?>
            <div class="col-md-4 col-lg-2">
                <div class="card shadow-sm oferta-card text-center">
                    <img src="<?= $p['imagen'] ?>" class="card-img-top" alt="Producto">
                    <div class="card-body">
                        <h6 class="fw-bold"><?= $p['nombre_producto'] ?></h6>
                        <p class="fw-bold text-success">₡<?= number_format($p['precio']) ?></p>
                        
                        <?php if (isset($_SESSION['rol']) && $_SESSION['rol'] === 'cliente'): ?>
                            <a href="carrito.php?id=<?= $p['id_producto'] ?>" class="btn btn-sm btn-primary">Agregar</a>
                        <?php endif; ?>
